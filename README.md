...
- `skill-management-with-symlinks`（根目录）：用软链接在项目组内安装 Skills，统一原件、节省上下文、一次更新多项目同步。
...
- `skill-management-with-symlinks` (root): install Skills project-locally with symbolic links, keep a single source of truth, save context window space, and update multiple projects with one upstream pull.
